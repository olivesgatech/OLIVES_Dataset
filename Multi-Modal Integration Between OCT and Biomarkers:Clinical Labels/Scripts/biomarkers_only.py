...
from biomarker_only_model import MLP
...
parser.add_argument('--which_baseline', type=str, default='att_only', choices=['img_only', 'att_only', 'img_att'])
...
def validateStandard(val_loader, model1, epoch, lossfn1, model2):
    ...
    with torch.no_grad():
        for i, (feat, label, idxs) in enumerate(val_loader): # for att_only
            if args.which_baseline == 'att_only':
                model2.zero_grad()
            elif args.which_baseline == 'img_only':
                model1.zero_grad()
...
def testStandard(classifier):
    ...
    with torch.no_grad():
        ...
        for i, (feat, label, idxs) in enumerate(testLoader):
            model.zero_grad()
...
def participants_classification(loader, model, opt, epoch, lossfn, att_model=None,
    ...
    # for i, (image, target, idxs, id) in enumerate(tbar):  # for img_att
    for i, (image, target, idxs) in enumerate(tbar): # for att_only
...
if __name__ == '__main__':
    ...
    BestStandardClassifier = '../Checkpoints/RCT_AttBaseline_Jun08_113750'  # att_only
